...
add_hook('AfterModuleSuspend', 1, function($vars) {
    try {
        ...
        mram_sms_send_notification('AfterModuleSuspend', $vars['userid'], $phone, [
            'client_name' => $client['client_name'],
            'service' => $hosting ? $hosting->domain : '',
            'domain' => $hosting ? $hosting->domain : '',
            'suspend_reason' => isset($vars['suspendreason']) ? $vars['suspendreason'] : '',
        ]);
    } catch (\Exception $e) { logActivity("MRAM SMS AfterModuleSuspend error: " . $e->getMessage()); }
});
